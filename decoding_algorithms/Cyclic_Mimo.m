...
codewordN = 32; % Codeword length (parfor needs local variable)
codewordK = 24; % Message length
...
parfor eb_n0_iter = 1:length(eb_n0)
    codewordN = 32; % Codeword length (parfor needs local variable)
    codewordK = 24; % Message length
    pol = cyclpoly(codewordN,codewordK);
    parmat = cyclgen(codewordN,pol);
    genmat = syndtable(parmat);
    information = randi(2,codewordK*4*frames,1)-1;
    encodedData = encode(information,codewordN,codewordK,'cyclic/binary',pol);
    %Use hamming code
    ...
    receivedBits =  decode(information_rx,codewordN,codewordK,'cyclic/binary',pol,genmat);
...
writematrix([eb_n0,ber],'data/CYCLIC_QAM16_sphere_decoding.txt');
