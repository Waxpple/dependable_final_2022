%samples=2400000;
function QAM64_KBEST(samples)
fprintf('64 QAM KBEST detection with '+string(samples)+' samples\n');
eb_n0 = (0:2:20)';
...
parfor eb_n0_iter = 1:length(eb_n0)
    ...
    for time_cnt = 1:length(symbol_tx)/4
        ...
        [Q,R] = qr(H);
        z = Q'*y;
        symbol_rx(4*(time_cnt-1)+1:4*(time_cnt)) = QAM64_KBEST_decoder(z,R);
    end
...
xlabel('Eb/N0(dB)');
ylabel('BER');
title('4x4 64 QAM KBEST detection with '+string(samples)+' samples');
axis([0 20 0.00001 1]);
writematrix([eb_n0,ber],'data/QAM64_KBEST.txt');
