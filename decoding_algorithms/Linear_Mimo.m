...
codewordN = 16; % Codeword length (parfor needs local variable)
codewordK = 8; % Message length
...
parfor eb_n0_iter = 1:length(eb_n0)
    ...
    encodedData = encode(information,codewordN,codewordK,'linear/binary',genmat);
    %Use hamming code
    ...
    receivedBits =  decode(information_rx,codewordN,codewordK,'linear/binary',genmat);
...
writematrix([eb_n0,ber],'data/LINEAR_QAM16_sphere_decoding.txt');
