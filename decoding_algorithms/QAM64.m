%samples=100;
function QAM64(samples)
fprintf('64 QAM detection with '+string(samples)+' samples\n');
eb_n0 = (0:2:20)';
...
parfor eb_n0_iter = 1:length(eb_n0)
    ...
    for time_cnt = 1:length(symbol_tx)/4
        ...
        kbest_z = kbest_Q'*y;
        kbest_symbol_rx(4*(time_cnt-1)+1:4*(time_cnt)) = QAM64_KBEST_decoder(kbest_z,kbest_R);
        %kbest_sorted
        ...
        kbest_sorted_symbol_rx(4*(time_cnt-1)+1:4*(time_cnt)) = kbest_sorted_O*QAM64_KBEST_decoder(kbest_sorted_z,kbest_sorted_R);
        %sd
        sd_symbol_rx(4*(time_cnt-1)+1:4*(time_cnt)) = QAM64_sphere_decoder(kbest_z,kbest_R);
        %sd_sorted
        sd_sorted_symbol_rx(4*(time_cnt-1)+1:4*(time_cnt)) = kbest_sorted_O*QAM64_sphere_decoder(kbest_sorted_z,kbest_sorted_R);

    end
    %zf
    zf_information_rx = QAM64_decoder(zf_symbol_rx,true);
    zf_err = length(find(zf_information_rx-information));
    zf_ber(eb_n0_iter) = zf_err/(samples*(eb_n0(eb_n0_iter)+1));
    %mmse
    mmse_information_rx = QAM64_decoder(mmse_symbol_rx,true);
    mmse_err = length(find(mmse_information_rx-information));
    mmse_ber(eb_n0_iter) = mmse_err/(samples*(eb_n0(eb_n0_iter)+1));
    %kbest
    kbest_information_rx = QAM64_decoder(kbest_symbol_rx,false);
    kbest_err = length(find(kbest_information_rx-information));
    kbest_ber(eb_n0_iter) = kbest_err/(samples*(eb_n0(eb_n0_iter)+1));
    %kbest
    kbest_sorted_information_rx = QAM64_decoder(kbest_sorted_symbol_rx,false);
    ...
    %sd
    sd_information_rx = QAM64_decoder(sd_symbol_rx,false);
    sd_err = length(find(sd_information_rx-information));
    sd_ber(eb_n0_iter) = sd_err/(samples*(eb_n0(eb_n0_iter)+1));
    %sd_sorted
    sd_sorted_information_rx = QAM64_decoder(sd_sorted_symbol_rx,false);
...
writematrix([eb_n0,zf_ber],'data/QAM64_zero_forcing.txt');
writematrix([eb_n0,mmse_ber],'data/QAM64_MMSE.txt');
writematrix([eb_n0,kbest_ber],'data/QAM64_KBEST.txt');
writematrix([eb_n0,kbest_sorted_ber],'data/QAM64_KBEST_sorted.txt');
writematrix([eb_n0,sd_ber],'data/QAM64_sphere_decoding.txt');
writematrix([eb_n0,sd_sorted_ber],'data/QAM64_sphere_decoding_sorted.txt');
end
