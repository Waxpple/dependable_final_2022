function QAM16_zero_forcing(samples)
fprintf('16 QAM Zero-forcing detection with '+string(samples)+' samples\n');
eb_n0 = (0:2:20)';
...
parfor eb_n0_iter = 1:length(eb_n0)
    ...
    information_rx = QAM16_decoder(symbol_rx,true);
...
xlabel('Eb/N0(dB)');
ylabel('BER');
title('4x4 16 QAM Zero-forcing detection with '+string(samples)+' samples');
writematrix([eb_n0,ber],'data/QAM16_zero_forcing.txt');
